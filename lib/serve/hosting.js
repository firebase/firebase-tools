  ...
  return hostingImplicitInit(options).then(function(init) {
    var server = superstatic({
      ...
      if (config.public && config.public !== '.') {
        logger.info('[hosting] Serving hosting files from:', config.public);
      }
      logger.info('[hosting] Local server: ' + chalk.underline(chalk.bold('http://' + options.host + ':' + options.port)) + '\n');
    });
    ...
    server.on('error', function(err) {
      if (err.code === 'EADDRINUSE') {
        ...
        if (_attempts < MAX_PORT_ATTEMPTS) {
          utils.logWarning('[hosting] ' + message + ' Trying another port...');
          options.port++;
          _attempts++;

...
var callableFunction = require('../callableFunction');
...
function _startEmulator(options) {
  ...
  var controllerConfig = _.merge({tail: true}, _getPorts(options)); // TODO: specify REST client
  ...
  .then(function(result) {
    ...
    return RSVP.all(_.map(operations, function(operation) {
      ...
      return emulatedFunctions;
    });
  }).then(function(emulatedFunctions) {

    var replServer = repl.start({
      prompt: 'functions > '
    });

    _.forEach(emulatedFunctions, function(funcName) {
      replServer.context[funcName] = new callableFunction(funcName, controller);
    });

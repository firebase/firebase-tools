...
function _startEmulator(options) {
  try {
    ...
    utils.logWarning('[functions] Cannot start emulator. ' + msg);
    return RSVP.reject();
  }
  if (process.version !== 'v6.9.1') {
    utils.logWarning('[functions] The emulator works best with Node version v6.9.1, you have ' + process.version + '\n');
  }

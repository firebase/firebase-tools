...
var Config = function(src, options) {
  ...
  if (this.has('rules') && this._hasDeepKey(this.get('rules'), '.function')) {
  ...
  if (this.has('functions')) {
    ...
    this.set('rules', triggerRulesGenerator(this.get('functions'), this.get('rules')));
  }

  // use 'public' as signal for legacy hosting since it's a required key
  if (!this.data.hosting && this._src.public || this._src.rules) {

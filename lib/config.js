...
var Config = function(src, options) {
  ...
  // load gcp service accounts for GCF
  if (_.has(this.data, ['functions', '.credentials'])) {
    if (this.projectDir) {
      var credentialsPath = path.join(this.projectDir, this.data.functions['.credentials']);
      this.data.functions['.credentials'] = loadCJSON(credentialsPath);
    }
  }
...
Config.TARGETS = ['hosting', 'rules', 'functions'];

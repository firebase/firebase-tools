...
var Config = function(src, options) {
  ...
  if (this.has('database.rules') && this._hasDeepKey(this.get('database.rules'), '.function')) {
    throw new FirebaseError('Cannot define .function in rules, please use functions config instead', {exit: 1});
  }

  if (this.has('functions')) {
    // note if no user-specified rules are present for warning
    if (!this.get('database.rules')) {
      this._noRulesProvided = true;
    }
    this.set('database.rules', triggerRulesGenerator(this.get('functions'), this.get('database.rules')));
  }

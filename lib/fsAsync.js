...
function _readdirRecursive(options) {
  return readdir(options.path).then(function(dirContents) {
    var work = _
      .chain(dirContents)
      .map(function(shortName) { return path.join(options.path, shortName); })
...
function readdirRecursive(options) {
  ...
  var rules = _.map(options.ignore || [], function(glob) { return minimatch.filter(glob, mmopts); });
  var filter = function(test) {
      return _.some(rules, function(rule) { return rule(test); });
  };
  return _readdirRecursive({
    path: options.path,
    filter: filter,

...
const REMOTE_ROOT_URL = "https://storage.googleapis.com/firebase-preview-drop/emulator";

const CACHE_DIR =
  process.env.FIREBASE_EMULATORS_PATH || path.join(userHome, ".cache", "firebase", "emulators");

function emulatorPath(emulator) {
  return path.join(CACHE_DIR, emulator.name);
}
function emulatorUrl(emulator) {
  return REMOTE_ROOT_URL + "/" + emulator.name;
}
const emulators = {
  database: {
    instance: null,
    name: "firebase-database-emulator-v3.4.0.jar",
    port: 9000,
    stdout: null,
  },
  firestore: {
    instance: null,
    name: "cloud-firestore-emulator-v1.0.0.jar",
    port: 8080,
    stdout: null,
  },
};
  ...
  return Promise.all(
    _.map(targetNames, targetName => {
      ...
      if (!options.yes) {
        next = prompt.once({
          type: "confirm",
          message: "Are you sure you want to download the " + targetName + " emulator?",
        });
      } else {
      ...
      return next
        .then(proceed => {
          if (!proceed) {
            return utils.reject("Download aborted by user.", { exit: 1 });
          }
        })
        .then(() => {
          return new Promise((resolve, reject) => {
            ...
            var req = request.get(emulatorUrl(emulator));
    ...
    .then(() => {
      return Promise.all(
        _.map(targetNames, targetName => {
          ...
          emulator.instance = childProcess.spawn(

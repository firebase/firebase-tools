...
var _validate = function(index) {
  ...
  if (!index.collectionId) {
    throw new FirebaseError(
      'Index must contain "collectionId": ' + indexString
    );
  }
  ...
  for (var i = 0; i < index.fields.length; i++) {
    ...
    if (!field.fieldPath) {
      throw new FirebaseError(
        'All index fields must contain "fieldPath": ' + indexString
      );
    }

    if (!field.mode) {
      throw new FirebaseError(
        'All index fields must contain "mode": ' + indexString
      );
    }

    if (VALID_INDEX_MODES.indexOf(field.mode) < 0) {
      throw new FirebaseError(
        "Index field mode must be one of " +
          VALID_INDEX_MODES.join(", ") +
          ": " +
          indexString
      );
    }
...
var prepare = function(context, options) {
  ...
  if (!parsedSrc.indexes) {
    throw new FirebaseError(
      'Indexes file must contain "indexes" property: ' + indexesPath
    );
  }

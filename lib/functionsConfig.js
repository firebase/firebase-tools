...
exports.getFirebaseConfig = function(options) {
  return getProjectNumber(options)
    .then(function(projectNumber) {
      return api.request(
        "GET",
        "/v1/projects/" + projectNumber + ":getServerAppConfig",
        {
          auth: true,
  ...
  _.forEach(args, function(arg) {
    ...
    if (parts.length < 2) {
      throw new FirebaseError(
        "Invalid argument " + chalk.bold(arg) + ", must be in key=val format"
      );
    }
    if (/[A-Z]/.test(key)) {
      throw new FirebaseError(
        "Invalid config name " + chalk.bold(key) + ", cannot use upper case."
      );
    }
    ...
    if (_.includes(exports.RESERVED_NAMESPACES, id.config.toLowerCase())) {
      throw new FirebaseError(
        "Cannot set to reserved namespace " + chalk.bold(id.config)
      );
    }
...
exports.parseUnsetArgs = function(args) {
  ...
  _.forEach(args, function(key) {
    ...
    if (_.includes(exports.RESERVED_NAMESPACES, id.config.toLowerCase())) {
      throw new FirebaseError(
        "Cannot unset reserved namespace " + chalk.bold(id.config)
      );
    }

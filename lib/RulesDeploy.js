...
RulesDeploy.prototype = {
  ...
  createRulesets: function() {
    ...
    _.forEach(this.rulesFiles, function(files, filename) {
      ...
      promises.push(
        gcp.rules
          .createRuleset(self.options.project, files)
  ...
  release: function(filename, resourceName) {
    ...
    return gcp.rules
      ...
      .then(function() {
        utils.logSuccess(
          chalk.bold.green(self.type + ": ") +
            "released rules " +
            chalk.bold(filename) +
            " to " +
            chalk.bold(resourceName)
        );
      });
  },
  ...
  _compileRuleset: function(filename, files) {
    ...
    var self = this;
    return gcp.rules
      .testRuleset(self.options.project, files)
      .then(function(response) {
        if (
          response.body &&
          response.body.issues &&
          response.body.issues.length > 0
        ) {

...
FunctionsEmulator.prototype.start = function(shellMode) {
  ...
  try {
    ...
    var msg = err;
    utils.logWarning(
      chalk.yellow("functions:") + " Cannot start emulator. " + msg
    );
    return RSVP.reject();
  }
  ...
  return functionsConfig
    ...
    .then(function() {
      ...
      return parseTriggers(projectId, functionsDir, {}, firebaseConfig).catch(
        function(e) {
          utils.logWarning(
      ...
      var promises = _.map(triggers, function(trigger) {
        if (trigger.httpsTrigger) {
          return controller
            ...
            .catch(function(e) {
              logger.debug(
                "Error while deploying to emulator: " + e + "\n" + e.stack
              );
    ...
    .then(function(operations) {
      return RSVP.all(
        _.map(operations, function(operation) {
          ...
          return _pollOperation(operation.value, controller).then(function(
            res
          ) {
            ...
            if (res.httpsTrigger) {
              ...
              var message =
                chalk.green.bold("functions: ") + funcName.replace(/\-/g, ".");
              ...
              var provider = utils.getFunctionsEventProvider(
                res.eventTrigger.eventType
              );
    ...
    .catch(function(e) {
      if (e) {
        utils.logWarning(
          chalk.yellow("functions:") + " Error from emulator. " + e
        );

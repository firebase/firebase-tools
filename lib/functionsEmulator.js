...
FunctionsEmulator.prototype.start = function(runningTerminal) {
  var runningTerminal = runningTerminal || false;
  ...
  .then(function(result) {
    ...
    var promises = _.map(triggers, function(trigger) {
      ...
      if (!runningTerminal) {
        return RSVP.resolve(); // Don't emulate non-HTTPS functions if terminal not running
      }
    ...
    return RSVP.all(_.map(operations, function(operation) {
      ...
      return _pollOperation(operation.value, controller).then(function(res) {
        ...
        if (res.httpsTrigger) {
          var msg = chalk.green.bold('functions: ') + funcName;
          if (!runningTerminal) {
            msg += ': ' + chalk.bold(res.httpsTrigger.url);
          }

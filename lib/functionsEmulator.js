...
FunctionsEmulator.prototype._setIdleFunctionsInterval = function() {
  console.log('In the emulator...');

  var timeoutms = _.multiply(this.options.timeout, 1000);
  var timeout = {
    maxIdle: timeoutms
  };

  // Default for --idlePruneInterval is 60 seconds. If timeout is less than 60 seconds, --idlePruneInterval needs to be changed.
  if (this.options.timeout <= 60) {
    timeout.idlePruneInterval = _.divide(timeoutms, 2);
  }
  console.log(timeout);
  return timeout;
};
...
FunctionsEmulator.prototype.start = function(shellMode) {
  ...
  console.log('looking at my options');
  ...
  var funcIntervals = this._setIdleFunctionsInterval();
  var controllerConfig = _.merge({tail: true, service: 'rest'}, ports, funcIntervals);

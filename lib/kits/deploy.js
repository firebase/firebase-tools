    ...
    .then(function(firebaseconfig) {
      ...
      return RSVP.all(
        _.map(functions, function(cloudfunction) {
          var functionTrigger = _getFunctionTrigger(
            cloudfunction,
            firebaseconfig,
            config
          );
...
module.exports = function(functions, options, config, sourceUploadUrl) {
  return _deployKitFunctions(functions, options, config, sourceUploadUrl).then(
    function(operations) {
      var printSuccess = function() {
        return utils.logSuccess(
          chalk.green.bold("kits: ") + "Your kit has successfully been deployed"
        );
      };

...
ProfileReport.prototype.collectUnindexed = function(data, path, unindexed) {
  if (!data.unIndexed) {
    return;
  }
...
ProfileReport.prototype.collect = function(data, path, bytes, speed, direction) {
  this.collectSpeed(data, path, speed);
  this.collectBandwidth(bytes, path, direction);
};
...
ProfileReport.prototype.processOperation = function(data) {
  ...
  case 'concurrent-disconnect':
    break;
  case 'realtime-read':
    this.collect(data, path, data.bytes, this.state.readSpeed, this.state.outband);
    break;
  case 'realtime-write':
    this.collect(data, path, data.bytes, this.state.writeSpeed, this.state.inband);
    break;
  ...
  case 'listener-listen':
    this.collect(data, path, data.bytes, this.state.readSpeed, this.state.outband);
    this.collectUnindexed(data, path, this.state.unindexed);
    break;
  case 'listener-broadcast':
    this.collect(data, path, data.bytes, this.state.broadcastSpeed, this.state.outband);
    break;
...
ProfileReport.prototype.renderUnindexedData = function(state) {
  var table = new Table({
  ...
  var unindexed = this.collapsePaths(state.unindexed, function(u1, u2) {
...
ProfileReport.prototype.renderBroadcaseSpeed = function() {
...
ProfileReport.prototype.generateRaw = function() {
  return this.parse(this.writeRaw, function() {
    return null;
  });
};
...
ProfileReport.prototype.generateText = function() {
  return this.parse(this.processOperation, this.outputText);
};
...
ProfileReport.prototype.generateJson = function() {
  return this.parse(this.processOperation, this.outputJson);
};

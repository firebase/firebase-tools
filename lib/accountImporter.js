...
var _genUploadAccountPostBody = function(projectId, accounts, hashOptions) {
  ...
  if (hashOptions.hashKey) {
    postBody.signerKey = new Buffer(hashOptions.hashKey)
...
var validateOptions = function(options) {
  ...
  switch (options.hashAlgo.toUpperCase()) {
  case 'HMAC_SHA256':
  case 'HMAC_SHA1':
  case 'HMAC_MD5':
    if (!options.hashKey || options.hashKey === '') {
      return utils.reject('Must provide hash key for hash algorithm ' + options.hashAlgo, {exit: 1});
    }
    return {hashAlgo: options.hashAlgo, hashKey: options.hashKey};
  case 'MD5':
  case 'PBKDF_SHA1':
    ...
    return {hashAlgo: options.hashAlgo, rounds: options.rounds};
  case 'SCRYPT':
    ...
    if (isNaN(memCost) || memCost <= 0 || memCost > 8) {
      return utils.reject('Must provide valid memory cost(1..14) for hash algorithm ' + options.hashAlgo, {exit: 1});
    }
    return {hashAlgo: options.hashAlgo, rounds: options.rounds, memCost: options.memCost};
  default:

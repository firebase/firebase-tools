...
RC.prototype = {
  ...
  removeSpecifiedTarget: function(project, type, name, resource) {
    var tp = ['targets', project, type, name];
    var updatedResources = this.get(tp, []).filter(function(r) {
      return r !== resource;
    });

    if (updatedResources.length) {
      this.set(tp, updatedResources);
    } else {
      this.unset(tp);
    }
  },

  applyTarget: function(project, type, name, resources) {
    if (!TARGET_TYPES[type]) {
      throw new FirebaseError(
        'Unrecognized target type ' + chalk.bold(type) +
        '. Must be ' + _.keys(TARGET_TYPES).join(', '),
        {code: 1}
      );
    }
    ...
    resources.forEach(function(resource) {
      ...
      if (cur && cur !== name) {
        this.removeSpecifiedTarget(project, type, cur, resource);
        changed.push({resource: resource, target: cur});
      }
    }.bind(this));

    // apply resources to new target
    var existing = this.get(['targets', project, type, name], []);
    var list = _.uniq(existing.concat(resources)).sort();
    this.set(['targets', project, type, name], list);
  ...
  removeTarget: function(project, type, resource) {
    ...
    this.removeSpecifiedTarget(project, type, name, resource);
    this.save();
    return name;
  },

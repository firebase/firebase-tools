...
RC.prototype = {
  ...
  requireTarget: function(project, type, name) {
    if (!this.target(project, type, name).length) {
      throw new FirebaseError('Deploy target ' + chalk.bold(name) + ' not configured for project ' + chalk.bold(project) + '. Configure with:\n\n  firebase target:apply ' + type + ' ' + name + ' <resources...>', {exit: 1});
    }
  },

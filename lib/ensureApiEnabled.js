...
var _checkEnabled = function(projectId, apiName, prefix, silent) {
  return api
    ...
    .then(function(response) {
      var isEnabled =
        _.get(response.body, "usageSettings.consumerEnableStatus") ===
        "ENABLED";
      if (isEnabled && !silent) {
        utils.logSuccess(
          chalk.bold.green(prefix + ":") + " all necessary APIs are enabled"
        );
      }
      return isEnabled;
    });
...
var _pollCheckEnabled = function(
  projectId,
  apiName,
  prefix,
  enablementRetries,
  ...
  return new RSVP.Promise(function(resolve) {
    ...
    return _checkEnabled(projectId, apiName, prefix).then(function(isEnabled) {
      ...
      utils.logBullet(
        chalk.bold.cyan(prefix + ":") + " waiting for APIs to activate..."
      );
      return _pollCheckEnabled(
        projectId,
        apiName,
        prefix,
        enablementRetries,
...
_enableApiWithRetries = function(
  projectId,
  apiName,
  prefix,
  enablementRetries
  ...
  if (enablementRetries > 1) {
    return utils.reject(
      "Timed out waiting for APIs to enable. Please try again in a few minutes."
    );
  }
  return _enableApi(projectId, apiName, prefix, enablementRetries).then(
    function() {
...
var _ensureApi = function(projectId, apiName, prefix, silent) {
  if (!silent) {
    utils.logBullet(
      chalk.bold.cyan(prefix + ":") + " ensuring necessary APIs are enabled..."
    );
  }
  return _checkEnabled(projectId, apiName, prefix, silent).then(function(
    isEnabled
  ) {
    if (isEnabled) {
      return true;

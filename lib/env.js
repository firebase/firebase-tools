...
function fetchLatest(projectId) {
  return fetchVariable(projectId, 'meta').then(function(meta) {
    ...
    return fetchVariable(projectId, meta.version).then(function(fetched) {
      if (!fetched) { // fetch will be null if user has deleted their env in another way
        return {
          version: 'v0',
          data: {}
        };
...
function setMetaVersion(projectId, version, latestValue) {
  return fetchVariable(projectId, 'meta', true).then(function(meta) {
    meta = meta || {};
    var newMeta = _.assign({}, meta, {
      version: version,
      latest: latestValue
    });

    return setVariable(projectId, 'meta', newMeta);
  });
}
...
exports.getLatestCustomVariables = function(projectId) {
  return ensureConfig(projectId).then(function() {
    return fetchLatest(projectId, true);
  }).then(function(fetched) {
    var dataWithoutFirebase = _.omit(fetched.data, 'firebase');
    var result = {
      data: dataWithoutFirebase,
...
exports.set = function(projectId, value, version) {
  return setVariable(projectId, version, value).then(function() {
    return setMetaVersion(projectId, version, value);
  });
};

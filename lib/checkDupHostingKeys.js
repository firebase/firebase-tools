...
module.exports = function(options) {
  return new RSVP.Promise(function(resolve, reject) {
    if (!options.config) {
      return reject(new FirebaseError('Missing firebase.json.'));
    }
    ...
    if (hasLegacyKeys && _.has(src, ['hosting'])) {
      ...
      logger.info('Please note that this will over-write any configuration keys nested inside the ' + chalk.bold('hosting') + ' key with configuration keys at the root level of ' + chalk.bold('firebase.json.'));

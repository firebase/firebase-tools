...
function _functionsBucketName(projectId) {
  return _getDefaultBucket(projectId).then(function(name) {
    return 'staging.' + name;
  });
}
...
function _getOrCreateBucket(projectId) {
  var bucketName;
  return _functionsBucketName(projectId).then(function(bucket) {
    bucketName = bucket;

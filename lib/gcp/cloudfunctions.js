...
function _createFunction(projectId, region, eventType, functionName, entryPoint, trigger, sourceArchiveUrl) {
  ...
  return api.request('POST', endpoint, {
    ...
    return RSVP.resolve({func: func, eventType: eventType, done: false, name: resp.body.name, type: 'create'});
  }, function(err) {
    return _functionsOpLogReject(functionName, 'create', err);
  });
}

function _updateFunction(projectId, region, functionName, entryPoint, trigger, sourceArchiveUrl) {
  var location = 'projects/' + projectId + '/locations/' + region;
  var func = location + '/functions/' + functionName;

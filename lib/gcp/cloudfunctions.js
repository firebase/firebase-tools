...
function _createFunction(projectId, region, functionName, topicName, gcsUrl) {
...
function _updateFunction(projectId, region, functionName, topicName, gcsUrl) {
  var location = 'projects/' + projectId + '/regions/' + region;
  var func = location + '/functions/' + functionName;
  var topic = 'projects/' + projectId + '/topics/' + topicName;
...
function _checkOperation(operation) {
  return api.request('GET', '/' + version + '/operations/' + operation.name, {
    ...
    if (resp.body.done) { operation.done = true;}

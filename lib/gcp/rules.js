...
function createRuleset(projectId, files) {
  ...
  return api.request('POST', '/v1/projects/' + projectId + '/rulesets', {
    auth: true,
    data: payload,
...
function createRelease(projectId, rulesetName, releaseName) {
  ...
  return api.request('POST', '/v1/projects/' + projectId + '/releases', {
    auth: true,
    data: payload,
    origin: api.rulesOrigin
  }).then(function(response) {
...
function updateRelease(projectId, rulesetName, releaseName) {
  ...
  return api.request('PUT', '/v1/projects/' + projectId + '/releases/' + releaseName, {
    auth: true,
    data: payload,
    origin: api.rulesOrigin
  }).then(function(response) {
...
function testRuleset(projectId, files) {
  return api.request('POST', '/v1/projects/' + encodeURIComponent(projectId) + ':test', {
    origin: api.rulesOrigin,
    data: {
      source: {files: files}
    },

...
module.exports = function(context, options, payload) {
  ...
  utils.logBullet(
    chalk.cyan.bold("hosting:") +
      " preparing " +
      chalk.bold(payload.hosting.public) +
      " directory for upload..."
  );
  return prepareUpload(options, payload.hosting).then(function(upload) {
    if (!upload.foundIndex) {
      utils.logWarning(
        chalk.bold("Warning:") + " Public directory does not contain index.html"
      );
    }

    if (!upload.manifest.length) {
      return utils.reject(
        "Must have at least one file in public directory to deploy.",
        { exit: 1 }
      );
    }
    ...
    return new RSVP.Promise(function(resolve, reject) {
        ...
        switch (status) {
          case "deploying":
            ...
            if (bar) {
              ...
              process.stdout.write(
                _.repeat(chalk.green("."), percent - lastPercent)
              );
              lastPercent = percent;
            }
            lastCount = uc;
          ...
          case "deployed":
            ...
            utils.logSuccess(
              chalk.green.bold("hosting:") +
                " " +
                upload.manifest.length +
          ...
          case "removed":
            reject(
              new FirebaseError("Not Implemented", {
                exit: 2,
                context: snap.val(),
              })
            );
      ...
      api
        .request(
          "PUT",
          "/v1/hosting/" + options.instance + "/uploads/" + local.versionId,
          {
            auth: true,
            query: {
        ...
        .then(function() {
          logger.debug(
            "[hosting] .tgz uploaded successfully, waiting for extraction"
          );

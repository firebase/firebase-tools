...
module.exports = function(context, options, payload) {
  ...
  var legacySourceUrlTwo =
    "gs://" +
    "staging." +
    context.firebaseConfig.storageBucket +
  ...
  function _functionMatchesGroup(functionName, groupChunks) {
    return _.isEqual(
      groupChunks,
      functionName.split("-").slice(0, groupChunks.length)
    );
  }
  ...
  function _pollAndManageOperations(operations) {
    ...
    var printFail = function(op) {
      ...
      failedDeployments += 1;
      utils.logWarning(
        chalk.bold.yellow("functions[" + op.functionName + "]: ") +
          "Deployment error."
      );
      if (op.error.code === 8) {
  ...
  return gcp.cloudfunctions
    ...
    .then(function(existingFunctions) {
      ...
      var releaseNames = _getReleaseNames(
        uploadedNames,
        existingNames,
      ...
      return _fetchTriggerUrls(successfulCalls)
        .then(function() {
          return _pollAndManageOperations(successfulCalls).catch(function() {
            ...
            return RSVP.reject(
              new FirebaseError(
                "Failed to get status of functions deployments."
              )
            );
          });
        })
        .then(function() {
          if (deployments.length > 0) {
            ...
            track(
              ...
              "success",
              deployments.length - failedDeployments
            );
          }
          ...
          if (failedDeployments > 0) {
            ...
            logger.info(
              "    " + chalk.bold("firebase deploy --except functions")
            );
            return RSVP.reject(

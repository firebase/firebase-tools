  ...
  return gcp.cloudfunctions.list(projectId, GCP_REGION).then(function(existingFunctions) {
    ...
    _.chain(uploadedNames)
      ...
      .forEach(function(functionName) {
        ...
        // Provide `availableMemory`, `timeout` and `failurePolicy` retrieved from `gcp.cloudfunctions.list`
        // until bug in PATCH API is fixed (69678272)
        deployments.push({
          functionName: functionName,
          retryFunction: function() {
            return gcp.cloudfunctions.update({
              ...
              trigger: functionTrigger,
              sourceArchiveUrl: sourceUrl,
              timeout: existingFunction.timeout,

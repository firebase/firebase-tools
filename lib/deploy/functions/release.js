...
module.exports = function(context, options, payload) {
  ...
  var legacySourceArchiveUrl = 'gs://' + context.functionsBucket + '/' + gcp.storage.archiveName;
  ...
  function _fetchTriggerUrls(ops) {
    ...
    return gcp.cloudfunctions.list(projectId, GCP_REGION).then(function(functions) {
      var httpFunctions = _.chain(functions).filter({ sourceUploadUrl: sourceUrl}).filter('httpsTrigger').value();
  ...
  function _pollAndManageOperations(operations) {
    ...
    if (_.size(operations) > 90) {
      utils.logWarning(chalk.bold.yellow('functions:') + ' too many functions are being deployed, cannot poll status.');
      logger.info('In a few minutes, you can check status at ' + utils.consoleUrl(options.project, '/functions/logs'));
    ...
    var printFail = function(op) {
      _endTimer(op.functionName);
      failedDeployments += 1;
      utils.logWarning(chalk.bold.yellow('functions[' + op.functionName + ']: ') + 'Deployment error.');
      if (op.error.code === 8) {
    ...
    return pollOperation.pollAndRetry(operations, pollFunction, interval, printSuccess, printFail, retryCondition);
  }
  ...
  return gcp.cloudfunctions.list(projectId, GCP_REGION).then(function(existingFunctions) {
    ...
    _.chain(uploadedNames)
      ...
      .forEach(function(functionName) {
        ...
        logger.debug('Trigger is: ', JSON.stringify(functionTrigger));
        var eventType = functionTrigger.eventTrigger ? functionTrigger.eventTrigger.eventType : 'https';
        var existingFunction = _.find(existingFunctions, { functionName: functionName });
        ...
        if (migratingTrigger) {
          throw new FirebaseError('Function ' + chalk.bold(functionName) + ' was deployed using a legacy ' +
            'trigger type and cannot be updated with the new SDK. To proceed with this deployment, you must first delete the ' +
            'function by visiting the Cloud Console at: https://console.cloud.google.com/functions/list?project=' + projectId +
    ...
    _.chain(existingFunctions)
      .filter(function(functionInfo) {
        if (typeof functionInfo.labels === 'undefined') {
          var originUploadUrl = functionInfo.sourceUploadUrl === sourceUrl;
          var originArchiveUrl = functionInfo.sourceArchiveUrl === legacySourceArchiveUrl || functionInfo.sourceArchiveUrl === legacySourceUrl;
          return originArchiveUrl || originUploadUrl;
        }

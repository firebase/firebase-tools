...
module.exports = function(context, options, payload) {
  if (options.config.has('functions')) {
    payload.functions = _.omit(options.config.get('functions'), ['.source']);
    var sourceDirName = options.config.get(['functions', '.source'], 'functions');

    if (!fsutils.dirExistsSync(resolveProjectPath(options.cwd, sourceDirName))) {
      var msg = sourceDirName === 'functions' ?
        chalk.bold('functions') + ' directory not found, must create or specify with .source' :
        chalk.bold(sourceDirName) + ' directory specified by functions .source does not exist, can\'t deploy functions';

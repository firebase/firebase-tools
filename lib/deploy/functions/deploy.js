...
module.exports = function(context, options, payload) {
  ...
  function _uploadSource(source) {
    // Functions upload endpoint does not support versions yet.
    return api.request('PUT', '/projects/' + context.projectId + '/functions/uploads/1', {
      auth: true,
      query: functionsConfig['.credentials'],
      files: {
        code: {
          filename: 'source.zip',

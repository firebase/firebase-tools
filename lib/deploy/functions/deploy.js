...
module.exports = function(context, options, payload) {
  var _uploadSource = function(source) {
    ...
    //   return gcp.storage.upload(source, bucketName);
    // });
    return gcp.storage.getUploadUrl(context.projectId, 'us-central1').then(function(uploadUrl){
      _.set(context, 'uploadUrl', uploadUrl);
      console.log('--------------FIIIIIIIIIIIIIRST HEREEEEEEEEEEEEEEEEEEEEE-------------------');
      console.log(context.uploadUrl);
      var uploadUrl = _.replace(uploadUrl, 'https://storage.googleapis.com', '');
      return gcp.storage.uploadtest(source, uploadUrl);
    })
  };

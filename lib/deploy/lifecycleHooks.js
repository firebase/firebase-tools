...
function checkAndRemoveEmptyCommands(commands, jsonFile) {
...
function runCommand(command, childOptions) {
  return new Promise(function(resolve, reject) {
    ...
    // option 1 use if/else statment in script
    // option 2 add check for $RESOURCE_DIR here and change based on necessity
    // option 3 use cross-env nodejs package
    if (command === '') {
      resolve();
    }
...
module.exports = function(target, hook) {
  ...
  return function(context, options) {
    ...
    checkAndRemoveEmptyCommands(commands, json);

...
var RESERVED_ENV = ['RESOURCE_DIR', 'PROJECT_DIR', 'GCLOUD_PROJECT'];

function runCommand(command, childOptions) {
  var translatedCommand = command;
  if (process.platform === 'win32') {
    RESERVED_ENV.forEach(function(env) {
      translatedCommand = translatedCommand.replace(new RegExp('$' + env, 'g'), '%' + env + '%');
    });
  }

  return new Promise(function(resolve, reject) {
    logger.info('Running command: ' + translatedCommand);

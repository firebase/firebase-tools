...
var deploy = function(targetNames, options) {
  ...
  for (var i = 0; i < targetNames.length; i++) {
    ...
    if (!target) {
      return RSVP.reject(
        new FirebaseError(
          chalk.bold(targetName) + " is not a valid deploy target",
          { exit: 1 }
        )
      );
    }
  ...
  logger.info();
  logger.info(
    chalk.bold(chalk.gray("===") + " Deploying to '" + projectId + "'...")
  );
  logger.info();
  ...
  return _chain(predeploys, context, options, payload)
    ...
    .then(function() {
      ...
      return api.request(
        "POST",
        "/v1/projects/" + encodeURIComponent(projectId) + "/releases",
        {
          data: payload,
          auth: true,

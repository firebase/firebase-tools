...
var _refreshAccessToken = function(refreshToken, authScopes) {
  ...
  return api.request('POST', '/oauth2/v3/token', {
    ...
    if (_.get(err, 'context.body.error') === 'invalid_scope') {
      throw new FirebaseError('This command requires new authorization scopes not granted to your current session. Please run ' + chalk.bold('firebase login --reauth'), {exit: 1});
    }

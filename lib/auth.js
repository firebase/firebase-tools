...
var auth = {
  ...
  requireLogin: function(callback) {
    ...
    if ((this.email.length === 0) || (this.token.length === 0)) {
      prompt.logger.warn('Login required');
      ...
      function(statusCode, response) {
        if (typeof(callback) === 'function') {
          if (response.success) {
            ...
            prompt.logger.error('OUTDATED CLI VERSION. Please ' +
                        'update to at least v' + response.minCLI);
          } else {
  ...
  _attemptLogin: function(tries, callback) {
    ...
    if (tries > 0) {
      if (tries !== this.maxRetries) {
        prompt.logger.warn('Email or password incorrect, please try again');
      }
  ...
  _handleLogin: function(callback, statusCode, response) {
    ...
    if (token) {
      ...
      // Always fatal, don't call callback
      prompt.logger.error('OUTDATED CLI VERSION. Please update to at least v' +
                            response.minCLI);
    } else {
      if (typeof(callback) === 'function') {

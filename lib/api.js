...
var api = {
  ...
  request: function(method, resource, data, authenticate, callback) {
    ...
    request(options, function(err, response, body) {
      if (err) {
        throw new FirebaseError('Server Error. Please try again', {
          original: err,
          context: {
            response: response,
            body: body
  ...
  getFirebases: function() {
    return new RSVP.Promise(function(resolve, reject) {
      api.request('GET', '/account', {}, true, function(statusCode, response) {
        if (response && response.firebases) {
          resolve(response.firebases);
        } else {
          reject(new FirebaseError('Server Error: Unexpected Response. Please try again', {
            context: {
              status: statusCode,
              body: response
            },

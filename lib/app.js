  ...
  deploy: function() {
    ...
    try {
      ...
      if (typeof(settings.firebase) === 'string') {
        Auth.checkCanAccess(settings.firebase, function(err, bbSite) {
          if (err) {
            console.log('You don\'t have access to this Firebase');
            return;
          }
          if (typeof(bbSite) !== 'undefined') {
          ...
          BitBalloon.deploy(settings, function(err, site) {
            if (err) {
              console.log('Couldn\'t deploy app');
              return;
            }
            console.log('Deployed. Check out your app at ' + site.url);
          });
        });
      }
    } catch (err) {

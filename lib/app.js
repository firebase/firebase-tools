...
module.exports = {
  init: function(argv) {
    ...
    auth.listFirebases(argv).then(function(res) {
      ...
      schemaPublic.conform = function(value) {
        if (path.relative('.', value).match(/^\./)) {
          schemaPublic.message = 'Please use a sub directory of current working directory as public directory';
          return false;
        }
        if (!fs.existsSync(value)) {
          schemaPublic.message = 'Please use an existing directory';
          return false;
        }
        return true;
      };

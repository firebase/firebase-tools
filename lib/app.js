...
var supportedTemplates = {
  angular: {
    ...
    completedMessage: chalk.green('Angular Template Successfully Added')
  },
  chat: {
    ...
    configRegex: /https:\/\/INSTANCE\.firebaseio\.com/,
    completedMessage: chalk.green('Chat Template Successfully Added')
  }
};
      ...
      var schema = [{
        ...
        beforeValue: function(results) {
          ...
          if (!argv.template || (typeof(argv.template) !== 'string') || !argv.template.match(templatePattern)) {
            console.log(chalk.yellow('----------------------------------------------------'));
            console.log(chalk.yellow('Available Bootstrapping Templates'));
            console.log(chalk.yellow('----------------------------------------------------'));
    ...
    auth.requireLogin(function(err) {
      ...
      auth.checkCanAccess(settings.firebase, function(err, tokens) {
        ...
        auth.updateRules(settings.firebase,
          ...
          upload.send(settings.firebase, settings['public'], directoryRef.name(), function(err, directory) {
            if (err) {

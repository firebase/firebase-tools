...
function getSettings(argv) {
  ...
  while (!fs.existsSync(path.resolve(curDir, './firebase.json'))) {
  ...
  defaultSettings.public = curDir;

  var settingsFile = path.resolve(curDir, './firebase.json');
    ...
    auth.requireLogin(argv, function(err) {
      ...
      auth.checkCanAccess(settings.firebase, function(err, tokens) {
        ...
        for (var i in defaultSettings) {
          if (i === 'public') {
            if (settings.hasOwnProperty('public')) {
              settings.public = path.resolve(defaultSettings.public, settings.public);
            } else {
              settings.public = defaultSettings.public;
            }
          } else if (defaultSettings.hasOwnProperty(i) &&
                !settings.hasOwnProperty(i)) {
            settings[i] = defaultSettings[i];
          }
        }

        if (path.relative(defaultSettings.public, settings.public).match(/^\.\./)) {

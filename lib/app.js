  ...
  init: function(argv) {
    auth.listFirebases().then(function(res) {
      ...
      if (fs.existsSync(settingsFile)) {
        console.log('Initialization error'.red + ' - Directory alread initialized');
        process.exit(1);
      }
      if (res.firebases.length === 0) {
        console.log('No Firebase apps'.red + ' - You need to create a Firebase app on https://firebase.com before you initialize an app for hosting');
        process.exit(1);
      }
      ...
      getPrompt(schema, function(results) {
        if (path.relative('.', results['public']).match(/^\./)) {
          console.log('Public Directory Error'.red + ' - Public directory must be within current working directory');
          process.exit(1);
        }
        if (!fs.existsSync(results['public'])) {
          console.log('Public Directory Error'.red + ' - Public directory does not exist');
          process.exit(1);
        }
        var settings = {
        ...
        auth.checkCanAccess(results.firebase, function(err) {
          if (err) {
            console.log('Permission Error'.red + ' - You do not have permission to use this Firebase');
            process.exit(1);
          }
    ...
    auth.requireLogin(function(err) {
      ...
      auth.checkCanAccess(settings.firebase, function(err, tokens) {
        ...
        if (fs.readdirSync(settings['public']).length == 0) {
          console.log('Public Directory Warning'.yellow + ' - Public ' +
        ...
        auth.updateRules(settings.firebase,
          ...
          upload.send(settings.firebase, settings['public'], function(err, directory) {
            if (err) {
              console.log('Deploy Error'.red + ' - Couldn\'t upload app');
              console.log(err);
              process.exit(1);
            }
            var bar = null;
            ...
            if (tokens.firebaseToken) {
              ...
              console.log('Upload Successful'.green + ' - Your upload is ' +

...
function uploadSite(settings, directoryRef, argv) {
  return function() {
    ...
    upload.send(settings.firebase, settings['public'], settings.ignore, directoryRef.key(), message, function(err, directory) {
      if (err) {
    ...
    _when.join(this.getTemplates(), auth.listFirebases(argv)).done(function(resultSet) {
      ...
      getPrompt(argv, schema, function(results) {
        ...
        extraction.on('entry', function(entry) {
          if (entry.type === 'File') {
            var key = path.normalize(entry.path).split(path.sep).slice(1).join(path.sep.replace(/\\/g, '\\\\'));
            var pattern = new RegExp('^' + templateRoot + '(.+)$');
  ...
  deleteSite: function(argv) {
    auth.requireLogin(argv, function(err) {
      ...
      auth.checkCanAccess(settings.firebase, function(err, tokens) {
        ...
        upload.deleteSite(settings.firebase, directoryRef.key(), message, function(err) {
          if (err) {
            console.log(chalk.red('Deploy Error') + ' - Couldn\'t upload app');
            console.log(err);
            process.exit(1);

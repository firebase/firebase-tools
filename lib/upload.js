  ...
  send: function(firebase, publicDir, callback) {
    ...
    writeStream.once('finish', function() {
      ...
      var r = request.put({
        ...
        fs.unlink(filename);
        setTimeout(callback, 0, err || !body.success, body ? body.directory : undefined);
      });
      var form = r.form();
      ...
      var bar = new ProgressBar('uploading  [:bar] :percent :etas', {
        ...
        total: fileCount.toString().length + form._overheadLength + form._boundary.length + form._valueLength + writeStream.bytesWritten + 3
      });

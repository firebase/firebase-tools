...
describe("Multiple Storage Deploy Targets", () => {
  ...
  const storageBucket = `${FIREBASE_PROJECT}.appspot.com`;
  const otherStorageBucket = `${FIREBASE_PROJECT}-2.appspot.com`;
  ...
  before(async function (this) {
    ...
    await test.applyTargets(["storage", "main", storageBucket]);
    await test.applyTargets(["storage", "other", otherStorageBucket]);
  ...
  it("should enforce different rules for different targets", async () => {
    ...
    const otherUploadURL = await supertest(STORAGE_EMULATOR_HOST)
      .post(
        `/v0/b/${otherStorageBucket}/o/test_upload.jpg?uploadType=resumable&name=test_upload.jpg`
      )
      .set({ "X-Goog-Upload-Protocol": "resumable", "X-Goog-Upload-Command": "start" })
      .expect(200)
      .then((res) => new URL(res.header["x-goog-upload-url"]));

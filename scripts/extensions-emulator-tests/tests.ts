...
describe("extension emulator", () => {
  ...
  before(async function (this) {
    ...
    const p = subprocess.spawnSync("firebase", ["--open-sesame", "extdev"], {
      cwd: __dirname,
    });
  ...
  it("should execute an HTTP function", async function (this) {
    ...
    const res = await test.invokeHttpFunction(
      TEST_FUNCTION_NAME,
      FIREBASE_PROJECT_ZONE
    );

    expect(res.status).to.equal(200);
    await expect(res.text()).to.eventually.equal(
      "Hello World from greet-the-world"
    );
  });
});

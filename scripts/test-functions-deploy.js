...
var deleteAllFunctions = function () {
  ...
  return (
    localFirebase +
    ` functions:delete ${toDelete.join(" ")} -f --project=${projectId}`
  );
};
...
var preTest = async function () {
  ...
  return functionsConfig
    .getFirebaseConfig({ project: projectId })
...
var postTest = function (errored) {
  ...
  if (!errored) {
    ...
    execSync(`${localFirebase} database:remove / -y --project=${projectId}`, {
      cwd: tmpDir,
    });
  }
...
var checkFunctionsListMatch = function (expectedFunctions) {
  ...
  return cloudfunctions
    ...
    .catch(function (err) {
      console.log(
        clc.red("Deployed functions do not match expected functions")
      );
...
var testCreateUpdate = function () {
  ...
  return new Promise(function (resolve) {
    exec(
      `${localFirebase} deploy --project=${projectId}`,
      { cwd: tmpDir },
      function (err, stdout) {
        console.log(stdout);
...
var triggerSchedule = function (job) {
  ...
  return api
    .request(
      "POST",
      `/v1/projects/${projectId}/locations/us-central1/jobs/${job}:run`,
      {
        auth: true,
        data: {},
...
var main = function () {
  preTest()
    ...
    .then(function () {
      console.log(
        clc.green("\u2713 Test passed: creating functions with filters")
      );
      return testDeleteWithFilter();
    })
    .then(function () {
      console.log(
        clc.green("\u2713 Test passed: deleting functions with filters")
      );

...
module.exports = new Command('kits:install <githubRepo>')
  ...
  .action(function(githubRepo, options) {
    ...
    return kits.prepareKitsUpload.retrieveFile(githubConfig).then(function(result) {
      var kitConfig = JSON.parse(result);
      kitFunctions = kitConfig.functions;

      utils.logSuccess(chalk.green.bold('kits: ') + 'Fetched configuration file from ' + chalk.bold(gitRepo));
      utils.logBullet(chalk.bold('We will now ask a series of questions to help set up your kit.'));
      ...
      utils.logBullet('Deploying kit ' + gitRepo + ' as ' + chalk.bold(runtimeConfig.kitname + '...'));

...
export async function getConfig(cwd: string) {
  ...
  const outDirPath = process.platform === "win32" &&  config.outDir.pathname.startsWith("/")
    ? config.outDir.pathname.substring(1)
    : config.outDir.pathname;
  const publicDirPath = process.platform === "win32" &&  config.publicDir.pathname.startsWith("/")

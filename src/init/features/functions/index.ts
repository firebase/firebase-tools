...
/**
 * Asks user questions to configure functions. Stores answers in setup.featureInfo.functions.
 */
export async function askQuestions(
  ...
  if (!config.src.functions) {
    ...
    setup.featureInfo.functions.initType = initOpt;
  }
}

/**
 * Executes the functions setup based on answers from askQuestions().
 */
export async function actuate(setup: any, config: Config, options: Options): Promise<any> {
  const initType = setup.featureInfo?.functions?.initType || "new";
  return initType === "new" ? initNewCodebase(setup, config) : overwriteCodebase(setup, config);
}

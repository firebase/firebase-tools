...
async function actuateWithInfo(
  ...
  // Create the initial SQL Connect Service and Schema generated by Gemini.
  ...
  try {
    // Generate the example SQL Connect Connector and seed_data.gql with Gemini.
    ...
    const [operationGql, seedDataGql] = await promiseWithSpinner(
      () =>
        Promise.all([
          generateOperation(PROMPT_GENERATE_CONNECTOR, serviceName, projectId, [saveSchemaGql]),
          generateOperation(PROMPT_GENERATE_SEED_DATA, serviceName, projectId, [saveSchemaGql]),
        ]),

...
export async function initGitHub(setup: Setup, config: any, options: any): Promise<void> {
  ...
  if (shouldWriteYMLHostingFile) {
    writeChannelActionYMLFile(
      YML_RELATIVE_PATH_PULL_REQUEST,
      githubSecretName,
      setup.projectId,
      script
    );
    ...
    logger.info();
    logSuccess(`Created workflow file ${bold(YML_RELATIVE_PATH_PULL_REQUEST)}`);
  }
  ...
  if (shouldWriteYMLDeployFile) {
    ...
    logger.info();
    logSuccess(`Created workflow file ${bold(YML_RELATIVE_PATH_MERGE)}...`);
  }
...
function mkdirNotExists(relativePath: string) {
  const fullPath = path.join(process.cwd(), relativePath);
  if (!dirExistsSync(fullPath)) {

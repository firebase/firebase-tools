...
const functionsEmulatorLogData = {
  metadata: {
    emulator: {
      name: "functions",
    },
...
export class FunctionsEmulator implements EmulatorInstance {
  ...
  async connect(): Promise<void> {
    ...
    watcher.on("change", (filePath) => {
      EmulatorLogger.log(
        "DEBUG",
        `File ${filePath} changed, reloading triggers`,
        functionsEmulatorLogData
      );
  ...
  addRealtimeDatabaseTrigger(
    ...
    return api
      ...
      .catch((err) => {
        EmulatorLogger.log("WARN", "Error adding trigger: " + err, functionsEmulatorLogData);
        throw err;
      });
  }
  ...
  private async handleBackgroundTrigger(req: express.Request, res: express.Response) {
    ...
    EmulatorLogger.log(
      "DEBUG",
      `Accepted request ${method} ${req.url} --> ${triggerId}`,
      functionsEmulatorLogData
    );
  ...
  private async handleHttpsTrigger(req: express.Request, res: express.Response) {
    ...
    EmulatorLogger.log(
      "DEBUG",
      `[functions] Runtime ready! Sending request!`,
      functionsEmulatorLogData
    );

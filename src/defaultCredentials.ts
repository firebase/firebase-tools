...
/**
 * Creates Cloud application default credentials using the authorization from
 * 'firebase login'.
 */
export class DefaultCredentials {
   ...
   */
  public async getPathAsync(): Promise<string | undefined> {
    const filePath = this.credFilePath();
    if (!filePath) {
  ...
  private getCredential(): RefreshTokenCredential | undefined {
  ...
  private credFilePath(): string | undefined {

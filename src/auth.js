...
var _loginWithoutLocalhost = function (userHint, authProvider) {
  if (authProvider === "GITHUB") {
    throw new FirebaseError(
      "GitHub integration currently requires localhost.",
      { exit: 1 }
    );
  }
        ...
        if (authProvider === "GITHUB") {
          if (query.code) {
            return _respondWithFile(
              ...
              res,
              200,
              "../templates/loginSuccessGithub.html"
            )
              .then(() => {
                server.close();
                return _getGithubTokensFromAuthorizationCode(
                  query.code,
                  callbackUrl
                );
              })

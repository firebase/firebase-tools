...
describe("dialogs", () => {
    ...
    describe("toMetadata", () => {
        ...
        it("handles fallback for legacy SAs", () => {
            ...
            expect(metadata).to.deep.equal([
                {location: "l", id: "legacy", accounts: dialogs.serviceAccountsForBackend("number", legacy)},
                {location: "l", id: "modernA", accounts: ["a"]},
            ]);
        });
    ...
    describe("selectBackendServiceAccounts", () => {
        ...
        it("handles no backends", async () => {
            ...
            await expect(dialogs.selectBackendServiceAccounts("number", "id", {})).to.eventually.deep.equal([]);
            expect(utils.logLabeledWarning).to.have.been.calledWith("apphosting", dialogs.WARN_NO_BACKENDS);
        });
        ...
        it("handles a single backend (opt yes)", async () => {
            ...
            await expect(dialogs.selectBackendServiceAccounts("number", "id", {})).to.eventually.deep.equal(["a"]);
        ...
        it("handles a single backend (opt no)", async () => {
            ...
            expect(utils.logLabeledBullet).to.have.been.calledWith("apphosting", dialogs.GRANT_ACCESS_IN_FUTURE);
        });
        ...
        it("handles multiple backends with the same (multiple) SAs (opt yes)", async () => {
            ...
            await expect(dialogs.selectBackendServiceAccounts("number", "id", {}))
                .to.eventually.deep.equal(dialogs.serviceAccountsForBackend("number", legacy));

            expect(prompt.confirm).to.have.been.calledWith({
                nonInteractive: undefined,
                default: true,
                message: "To use this secret, your backend's service account must have secret accessor permission. " +
        ...
        it("handles multiple backends with different SAs (select some)", async () => {
            ...
            await expect(dialogs.selectBackendServiceAccounts("number", "id", {})).to.eventually.deep.equal(["a", "b"])
            ...
            expect(utils.logLabeledBullet).to.not.have.been.calledWith("apphosting", dialogs.GRANT_ACCESS_IN_FUTURE);
        });

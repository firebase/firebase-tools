...
import * as prompt from "../../prompt";
  ...
  describe("checkSelectedEventsResponse", () => {
    ...
    it("should return false if allowed events is not part of extension spec's events list", () => {
      ...
      expect(
        logWarningSpy.calledWith("Unexpected event type 'google.firebase.nonexistent-event-occurred' was marked as allowed to be emitted. This event type is not part of the extension spec.")
      ).to.equal(true);
    });
  ...
  describe("askForAllowedEventTypes", () => {
    ...
    it("should keep prompting user until valid input is given", async () => {
      ...
      await askForAllowedEventTypes([{ type: "google.firebase.custom-event-occurred", description: "A custom event occurred" },]);

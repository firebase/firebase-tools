...
var prepareFirebaseRules = function (component, options, payload) {
  ...
  if (rulesPath) {
    ...
    utils.logBullet(
      clc.bold.cyan(component + ":") +
        " checking rules for compilation errors..."
    );
    return api
      .request(
        "POST",
        "/v1/projects/" + encodeURIComponent(options.project) + ":test",
        {
      ...
      .then(function (response) {
        ...
        utils.logSuccess(
          clc.bold.green(component + ":") + " rules file compiled successfully"
        );
        payload[component] = {

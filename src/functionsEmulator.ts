...
class FunctionsEmulator {
  ...
  async start(...args: any[]) {
    ...
    const functionsDir = path.join(this.options.config.projectDir, this.options.config.get("functions.source"));
    ...
    const admin_resolve = require.resolve("firebase-admin", {paths: [path.join(functionsDir, "node_modules")]});
    const admin_mock = {
      initializeApp() {
        if (!initializeAppWarned) {
          console.log(`${clc.yellow("[HUB]")} Your code attempted to use "admin.initializeApp()" we've ignored your options and provided an emulated app instead.`);
    ...
    const triggersByName = triggers.reduce((triggersByName: {[trigger_name: string]: any}, trigger: any) => {
      trigger.getRawFunction = () => {
        ...
        if (old_str !== new_str) {
          console.log(`${clc.blue("[FUNCTION:FIRESTORE]")} Your function "${trigger.name}" has been changed, here's the diff...`);
        ...
        console.log(`${clc.blue("[FUNCTION:FIRESTORE]")} Your function "${trigger.name}" will be invoked, here's the logs...`);
        return new_function
      };
    ...
    hub.get("/functions/projects/:project_id/triggers/:trigger_name", (req, res) => {
      console.log(`${clc.blue("[FUNCTION:HTTP]")} GET request to function ${req.params.trigger_name} accepted.`);
      const trigger = triggersByName[req.params.trigger_name];
      if (trigger.httpsTrigger) {
        trigger.getRawFunction()(req, res)
      } else {
        res.json({status: "error", message: "non-HTTPS trigger must be invoked with POST request"})
      }
    });
    ...
    hub.post("/functions/projects/:project_id/triggers/:trigger_name", (req, res) => {
      ...
      if (trigger.httpsTrigger) {
        ...
        const newSnap = proto.data.value && (app.firestore() as any).snapshot_(proto.data.value, new Date().toISOString(), 'json');
        const oldSnap = proto.data.oldValue && (app.firestore() as any).snapshot_(proto.data.oldValue, new Date().toISOString(), 'json');
    ...
    this.server = hub.listen(5000, () => {
      ...
      Object.keys(triggersByName).forEach((name) => {
        ...
        request.put(`http://localhost:8080/emulator/v1/projects/${projectId}/triggers/${name}`, {
          body: bundle
        }, (err, res, body) => {
          if (err) {
            console.warn(body);
          ...
          if (JSON.stringify(JSON.parse(body)) == "{}") {
            console.log(`${clc.green("[FIRESTORE]")} Trigger "${name}" has been acknowledged by the Firestore emulator`);
          }
        });
      });
    });
...
function _extractParamsFromPath(wildcardPath: string, snapshotPath: string) {
  ...
  return wildcardChunks.slice(-snapshotChucks.length).reduce((params: {[key: string]: string}, chunk, index) => {

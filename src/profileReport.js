 ...
 */
ProfileReport.prototype.collapsePaths = function (
  pathedObject,
  combiner,
  pathIndex
) {
  ...
  pathSegments.forEach(function (segments) {
    ...
    if (prefixCount.size >= COLLAPSE_THRESHOLD) {
      ...
      var collapsedPath = ProfileReport.pathString(
        prefix.concat(COLLAPSE_WILDCARD).concat(tail)
      );
      var currentValue = collapsedObject[collapsedPath];
      if (currentValue) {
        collapsedObject[collapsedPath] = combiner(
          currentValue,
          pathedObject[originalPath]
        );
      } else {
 ...
 * `path` table column.
 */
ProfileReport.prototype.renderUnpathedOperationSpeed = function (
  speedData,
  hasSecurity
) {
  ...
  if (Object.keys(speedData).length > 0) {
    var row = [
      ...
      ProfileReport.formatNumber(
        speedData.pendingCount === 0
          ? 0
          : speedData.pendingTime / speedData.pendingCount
      ) + " ms",
    ];
    if (hasSecurity) {
...
ProfileReport.prototype.renderOperationSpeed = function (
  pureData,
  hasSecurity
) {
  var head = [
...
ProfileReport.prototype.generate = function () {
  ...
  return utils.reject(
    new FirebaseError(
      'Invalid report format expected "TXT", "JSON", or "RAW"',
      {
        exit: 1,
      }
    )
...
ProfileReport.prototype.outputJson = function () {
  ...
  var json = {
    ...
    downloadedBytes: tableToJson(
      this.renderOutgoingBandwidth(),
      BANDWIDTH_NOTE
    ),

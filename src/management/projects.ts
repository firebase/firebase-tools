...
export const PROJECTS_CREATE_QUESTIONS: Question[] = [
  {
    ...
    message:
      ...
      `(${clc.yellow(
        "warning"
      )}: cannot be modified afterward) [6-30 characters]:\n`,
  },
  {
    type: "input",
    name: "displayName",
...
function logNewFirebaseProjectInfo(projectInfo: FirebaseProjectMetadata): void {
  ...
  logger.info(
    `https://console.firebase.google.com/project/${clc.bold(
...
export async function promptAvailableProjectId(): Promise<string> {
  const { projects, nextPageToken } = await getAvailableCloudProjectPage(
    MAXIMUM_PROMPT_LIST
  );
  if (projects.length === 0) {
    throw new FirebaseError(
...
export async function getFirebaseProjectPage(
  ...
  try {
    projectPage = await getProjectPage<FirebaseProjectMetadata>(
      "/v1beta1/projects",
      {
        responseKey: "results",
        pageSize,
...
export async function getAvailableCloudProjectPage(
  ...
  try {
    return await getProjectPage<CloudProjectInfo>(

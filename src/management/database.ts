...
export async function requireInstanceDetails(options: any): Promise<void> {
  return getDatabaseInstanceDetails(options.projectId, options.instance).then(details => {options.instanceDetails = details});
}

export async function getDatabaseInstanceDetails(projectId: string, instanceName: string) : Promise<DatabaseInstance> {
  try {
    ...
    return utils.reject(`Error while getting instance details for instance: ${instanceName} in project: ${projectId}. See firebase-debug.log for more details.`, {
      code: 2,
    })                           
  }
}
...
export async function getDefaultDatabaseInstance(projectId: string) : Promise<DatabaseInstance> {
  ...
  throw new FirebaseError(
    `No default RTDB instances exist for project ${projectId}`,
    {
      exit: 2
    }
  );
...
interface IterationResult { done: boolean, value: DatabaseInstance[] }
class DatabaseInstancesIterator  {
  ...
  public async next(): Promise<IterationResult> {
      ...
      try {
        ...
        logger.debug(err.message);
        return utils.reject(`Error while iterating over database instances for project: ${this.projectId}. See firebase-debug.log for more details.`, {
          code: 2,
        })                           
    }
  }

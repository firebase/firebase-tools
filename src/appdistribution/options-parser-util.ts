...
//Gets project name from tester input
export async function getProjectName(options: any): Promise<string> {
  const projectNumber = await needProjectNumber(options);
  return `projects/${projectNumber}`;
}

//Gets app name from tester input
export async function getAppName(options: any): Promise<string> {
  if (!options.app) {
  ...
  return `${projectName}/apps/${options.app}`;
}

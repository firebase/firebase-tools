...
5. Check if there is an active Firebase project for this environment (the \`firebase_get_environment\` tool may be helpful). If there is, proceed using that project. If there is not an active project, give the user two options: Provide an existing project id or create a new project. Only use the list_projects tool on user request. Wait for their response before proceeding.

...
5. Ensure there is an active Firebase App for their platform
   ...
   - Do the following only for Flutter apps
     - Execute \`firebase --version\`  to check if the Firebase CLU is installed
       - If it isn't installed, run \`npm install -g firebase-tools\` to install it. If it isn installed, skip to the next step. 

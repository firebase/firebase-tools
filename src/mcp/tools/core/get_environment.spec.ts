...
Confirm with the user before taking action.`;
    expect((result.content[0] as any).text).to.equal(expectedOutput);
  });
...
\`\`\``;
  ...
  it("should handle a single alias", async () => {
    ...
    const result = await get_environment.fn({}, options);
    expect((result.content[0] as any).text).to.include(
      "Active Project ID: test-project (alias: my-alias)",
    );
    expect((result.content[0] as any).text).to.include(
      `Available Project Aliases (format: '[alias]: [projectId]'): 
...
`,
  ...
  it("should handle multiple accounts", async () => {
    ...
    const result = await get_environment.fn({}, options);
    expect((result.content[0] as any).text).to.include("Authenticated User: test@example.com");
    expect((result.content[0] as any).text).to.include(`Available Accounts: 
...
`);
  ...
  it("should handle a single detected app", async () => {
    ...
    const result = await get_environment.fn({}, options);
    expect((result.content[0] as any).text).to.include(`Detected App IDs: 

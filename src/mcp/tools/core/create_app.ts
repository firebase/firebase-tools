...
export const create_app = tool(
  {
    ...
    inputSchema: z.object({
      displayName: z
        .string()
        .optional()
        .describe("The user-friendly display name for your app."),
      platform: z.enum(["web", "ios", "android"])
        .describe("The platform for which to create an app."),
      androidConfig: z.object({
  ...
  async ({ displayName, platform, androidConfig, iosConfig }, { projectId }) => {
    ...
    try {
      switch (platform) {
        case "android":
          return toContent(await createAndroidApp(projectId!, {
            displayName,
            packageName: androidConfig!.packageName,
          }));
        case "ios":
          return toContent(await createIosApp(projectId!, {
            displayName,
            bundleId: iosConfig!.bundleId,
            appStoreId: iosConfig!.appStoreId,
          }));
        case "web":
          return toContent(await createWebApp(projectId!, {
            displayName,
          }));
      }
    } catch (err: any) {

...
describe("create_app tool", () => {
  ...
  it("should create an Android app", async () => {
    ...
    const result = await (create_app as any)._fn(
  ...
  it("should create a Web app", async () => {
    ...
    const result = await (create_app as any)._fn(
      { display_name: displayName, platform: "web" },
      { projectId },
    );
  ...
  it("should throw an error if Android config is missing", async () => {
    await expect(
      (create_app as any)._fn({ platform: "android" }, { projectId }),
    ).to.be.rejectedWith("Android configuration is required when platform is 'android'");
  });

  it("should throw an error if iOS config is missing", async () => {
    await expect((create_app as any)._fn({ platform: "ios" }, { projectId })).to.be.rejectedWith(

...
describe("fetch_logs tool", () => {
  ...
  it("should return message if backendId is not specified", async () => {
    const result = await (fetch_logs as any)._fn({}, { projectId });
  ...
  context("when buildLogs is false", () => {
    it("should fetch service logs successfully", async () => {
      ...
      const result = await (fetch_logs as any)._fn({ backendId, location }, { projectId });
    ...
    it("should throw FirebaseError if service name cannot be determined", async () => {
      ...
      await expect(
        (fetch_logs as any)._fn({ backendId, location }, { projectId }),
      ).to.be.rejectedWith(FirebaseError, "Unable to get service name from managedResources.");
    });
  });
  ...
  context("when buildLogs is true", () => {
    ...
    it("should fetch build logs successfully", async () => {
      ...
      const result = await (fetch_logs as any)._fn(
        { buildLogs: true, backendId, location },
        { projectId },
      );
    ...
    it("should throw FirebaseError if build ID cannot be determined from buildLogsUri", async () => {
      ...
      await expect(
        (fetch_logs as any)._fn({ buildLogs: true, backendId, location }, { projectId }),

...
export async function getBackend(
  ...
  if (backends.length > 1) {
    const regions = backends.map((b) => apphosting.parseBackendName(b.name).location);
    throw new FirebaseError(
      `You have multiple backends with the same ${backendId} ID in regions: ${regions.join(", ")}. This is not allowed until we can support more locations. ` +

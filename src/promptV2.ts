...
export function guard<T>(opts: BasicOptions<T>): { shouldReturn: true, value: T } | { shouldReturn: false, value: undefined } {
    ...
    throw new FirebaseError(`Question "${opts.message}" does not have a default and cannot be answered in non-interactive mode`);
}
...
export type CheckboxOptions<Value> = BasicOptions<Value[]> & {
    message: string;
    choices: readonly (MaybeLiteral<Value> | inquirer.Separator)[] | readonly (inquirer.Separator | Choice<Value>)[];

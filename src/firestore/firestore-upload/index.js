 ...
 */
function uploadJSON(serviceAccountKeyFilePath, dataFilePath, targetCollectionName, firestoreDatabaseURL, dockeyInData) {
	...
	if ( (dataFile) && (typeof dataFile === "object") ) 
	{
		...
		if(dockeyInData === "true") 
		{
			...
			Object.keys(dataFile).forEach(docKey => {
				...
				firestore.collection(targetCollectionName).doc(docKey).set(dataFile[docKey]).then((res) => {
		...
		else
		{
			...
			Object.keys(dataFile).forEach(docKey => {
				//The .set() method is used to perform the upload
				 firestore.collection(targetCollectionName).doc().set(dataFile[docKey]).then((res) => {

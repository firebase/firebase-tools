...
async function initiateAppAndroidShaDeletion(
  ...
  try {
    ...
    certificateData = await deleteAppAndroidSha(projectId,appId, shaId);
    spinner.succeed();
  } catch (err) {
    spinner.fail();
    throw err;
...
module.exports = new Command("apps:android:sha:delete [appId] [shaId]")
  ...
  .action(
    async (
      ...
      logger.info(`Delete your SHA certificate hash with SHA id ${clc.bold(shaId)} in Android app ${clc.bold(appId)}:`);

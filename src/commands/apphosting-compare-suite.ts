...
export const command = new Command("apphosting:compare-suite")
  ...
  .option(
    "--suite-config <configPath>",
    "path to comparison suite JSON configuration file"
  )
  .option(
  ...
  .action(async (options: Options) => {
    ...
    if (!configPath) {
      throw new FirebaseError("Must specify --suite-config file containing the list of apps to compare.");
    }
      ...
      try {
        await runCompareSuite(
          ...
          testCase.pathB,
          {
            ...
            localBuildA: !!testCase.localBuildA,
            localBuildB: !!testCase.localBuildB
          }
        );
      } catch (err: any) {
        logger.error(`Test case ${testCase.name} failed: ${err.message}`);
      }
    }
  });

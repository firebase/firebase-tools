...
export function initAction(feature: string, options: CommandOptions): Promise<any> {
  ...
  if (!options.extensionOptions && process.platform === "win32") {
    next = promptOnce({
      type: "confirm",
  ...
  return next
    .then((proceed) => {
      ...
      if (options.extensionOptions) {
        // If extension calls this, it must specify a feature.
        return undefined;
      } else {
        return prompt(setup, [
          {

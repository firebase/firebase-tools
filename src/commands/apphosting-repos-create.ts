...
import {createGitRepoLink} from "../apphosting";
...
export const command = new Command("apphosting:repos:create")
  ...
  .option("-g, --gitconnection <connection>", "id of the connection (defaults to autogenerating a random id)", "")
.before(ensureApiEnabled)
  ...
  .action(async (options: Options) => {
    ...
    await createGitRepoLink(
      projectId,
      location as string|null,
      connection as string|null
    );
  });

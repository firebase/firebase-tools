...
export const command = new Command("dataconnect:execute [file] [operationName]")
  .description(
    `execute a SQL Connect query or mutation. If ${Constants.FIREBASE_DATACONNECT_EMULATOR_HOST} is set (such as during 'firebase emulator:exec'), executes against the emulator instead.`,
  )
  .option(
    "--service <serviceId>",

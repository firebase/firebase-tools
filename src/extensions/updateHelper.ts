...
function invalidSourceErrMsgTemplate(
  instanceId: string,
  source: string
): string {
  ...
  )}\` to update from the published source.\n
...
export async function getExistingSourceOrigin(
  ...
  try {
    const registryEntry = await resolveSource.resolveRegistryEntry(
      extensionName
    );
...
export async function updateFromLocalSource(
  ...
  try {
    ...
    throw new FirebaseError(
      invalidSourceErrMsgTemplate(instanceId, localSource)
    );
  }
  utils.logLabeledBullet(
    logPrefix,
...
export async function updateToVersionFromPublisherSource(
  ...
  utils.logLabeledBullet(
    logPrefix,
    `${clc.bold(
      "You are updating this extension instance to a published source."
    )}`
  );
  await showUpdateVersionInfo(
    instanceId,
...
export async function updateToVersionFromRegistry(
    ...
    if (version !== "latest" && semver.gt(minVer, version)) {
      throw new FirebaseError(
        `The version you are trying to upgrade to (${clc.bold(
          version
        )}) is less than the minimum version required (${clc.bold(
          minVer
  ...
  return resolveSource.resolveSourceUrl(
    registryEntry,
    existingSpec.name,
    targetVersion
  );
...
export async function updateFromRegistry(
  ...
  return updateToVersionFromRegistry(
    projectId,
    instanceId,
    existingSpec,
    existingSource,

...
function makeEventName(name: string, prefix: string): string {
  ...
  var match = name.match(versionedEvent);
  if (match) {
...
export async function writeSDK(
  ...
  if (!dirPath) {
    ...
    throw new FirebaseError("Invalid extension definition. Must have either extensionRef or localPath");
  }
  ...
  if (extensionRef) {
    sdkLines.push(` *   "Extensions SDK v${SDK_GENERATION_VERSION} for ${spec.name}@${spec.version}"`);
  } else {
  ...
  if (hasEvents) {
    ...
    //  sdkLines.push(`  ${makeEventName(event.type, prefix)}Handler?:  CloudFunction<CloudEvent<unknown>>;`);
    //}
  }
  if (extensionRef) {

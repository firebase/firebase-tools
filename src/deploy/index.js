...
var deploy = function (targetNames, options, customContext = {}) {
  ...
  for (var i = 0; i < targetNames.length; i++) {
    ...
    if (!target) {
      return Promise.reject(
        new FirebaseError(
          clc.bold(targetName) + " is not a valid deploy target",
          { exit: 1 }
        )
      );
    }
  ...
  logger.info();
  logger.info(
    clc.bold(clc.white("===") + " Deploying to '" + projectId + "'...")
  );
  logger.info();
  ...
  return _chain(predeploys, context, options, payload)
    ...
    .then(function () {
      ...
      logger.info(
        clc.bold("Project Console:"),
        utils.consoleUrl(options.project, "/overview")
      );
      if (deployedHosting) {
        _.each(context.hosting.deploys, function (deploy) {
          logger.info(
            clc.bold("Hosting URL:"),
            utils.addSubdomain(api.hostingOrigin, deploy.site)
          );
        });

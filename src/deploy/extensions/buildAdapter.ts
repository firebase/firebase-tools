...
import { ExtensionSpec, ExtensionConfig, Param as ExtensionParam, ParamType, Resource, FunctionResourceProperties, FunctionV2ResourceProperties } from "../../extensions/types";
  ...
  for (const resource of extensionSpec.resources) {
    if (resource.type === "firebaseextensions.v1beta.function" || 
      ...
      const endpoint: build.Endpoint = {
        ...
        region: getLocationFromResource(resource) ? [getLocationFromResource(resource)!] : ["us-central1"], // Could be ["${LOCATION}"]
...
function extractTriggerFromResource(resource: Resource): build.HttpsTriggered | build.EventTriggered {
  if (resource.type === "firebaseextensions.v1beta.function") {
    const props = resource.properties as FunctionResourceProperties["properties"];
...
function getEffectiveDefault(
  ...
  if (existingConfig) {
    const existingValue = existingConfig.params[extParam.param] || 
                         existingConfig.systemParams[extParam.param];
    if (existingValue) {
...
function convertExtensionInputToFunctionInput(
  extParam: ExtensionParam
): any | undefined {
...
export function backendToDeploymentInstanceSpec(
  ...
  const allResolvedParams = Object.fromEntries(
    Object.entries(envs)
      .filter(([name, paramValue]) => !paramValue.internal) // Skip internal Firebase params

...
export class AuthBlockingService implements Service {
  ...
  private async registerTriggerLocked(
    ...
    newBlockingConfig.forwardInboundCredentials = {
      idToken: (endpoint.blockingTrigger.options?.idToken as boolean) || false,
      accessToken: (endpoint.blockingTrigger.options?.accessToken as boolean) || false,
      refreshToken: (endpoint.blockingTrigger.options?.refreshToken as boolean) || false,
    };

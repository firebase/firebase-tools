...
// Helper to package v1 and/or v2 sources for a backend from a given directory
...
>>>>>>> origin
  ...
  for (const [codebase, wantBuild] of Object.entries(wantBuilds)) {
    ...
    let userEnvOpt: functionsEnv.UserEnvsOpts | undefined;
...
export async function loadCodebases(
  ...
  for (const codebase of codebases) {
    ...
    // Determine source directory: local source or remote clone
    ...
    // Create a runtime delegate (safe mode for remote), validate and discover

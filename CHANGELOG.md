...
* **BREAKING:** Increases the minimum version of `firebase-admin` in the Cloud Functions for Functions emulator from `8.0.0` to `8.9.0`.
* **BREAKING:** Increases the minimum version of `firebase-functions` in the Cloud Functions for Functions emulator from `3.0.0` to `3.3.0`.
* **BREAKING:** Removes support for top-level `hosting` config.
* **BREAKING:** `firebase serve` can no longer start the Cloud Firestore or Realtime Database emulators.
* **BREAKING:** Unifies the Cloud Functions for Functions emulator within `firebase serve` and `firebase emulators:start`.

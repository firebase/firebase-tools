...
async function _ga4Track(args) {
    ...
    const body = {
        ...
        events: [
            {
                name: eventName,
                params: Object.assign({ session_id: session.sessionId, 
                    ...
                    engagement_time_msec: (duration !== null && duration !== void 0 ? duration : 0).toFixed(3).replace(".", "").replace(/^0+/, ""), 
                    // https://support.google.com/analytics/answer/7201382?hl=en
                    // To turn debug mode off, `debug_mode` must be left out not `false`.
                    debug_mode: session.debugMode ? true : undefined, command_name: session.commandName }, params),
            },
        ],
    };
